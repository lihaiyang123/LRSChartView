...
- (CGFloat)spaceValue:(NSArray *)array{
    ...
    CGFloat max = [self getNumber:maxValue];
    
    if (_unitStyle == LRSUnitMillion) {
        if (self.max && self.min) {
            CGFloat maxValueInterger = (self.max - self.min) / 100000000.f;
            CGFloat xMaxValue = [self maxValueFloat:maxValueInterger];
            return xMaxValue / (_row - 1) * 100000000.f;
        }else{
            NSInteger maxValueInterger = max / 100000000.f + 0.99;
            NSInteger xMaxValue = [self maxValue:maxValueInterger];
            return xMaxValue / (_row - 1) * 100000000.f;
        }
    }else if (_unitStyle == LRSUnitWan) {
        
        if (self.max && self.min){
            CGFloat maxValueInterger = (self.max - self.min) / 10000.f;
            CGFloat xMaxValue = [self maxValueFloat:maxValueInterger];
        ...
        if (self.max && self.min){
            CGFloat maxValueInterger = (self.max - self.min) / 1000.f;
            CGFloat xMaxValue = [self maxValueFloat:maxValueInterger];
            return xMaxValue * 1000.f;
        }else{
        ...
        if (self.max && self.min) {
            CGFloat maxValueInterger = 0;
            max = self.max - self.min;
            if (max < 100000000) {
                maxValueInterger = max / 1000.f;
            }else{
                maxValueInterger = max / 10000000.f;
            }
    ...
    }else if (_unitStyle == LRSUnitScore){
        NSInteger xMaxValue = 5;
        return (xMaxValue - self.min) / (_row - 1);
    }else{
        if (self.max && self.min) {
            CGFloat maxValueInterger = 0;
            max = self.max - self.min;
            if (max >= 1000) {
                maxValueInterger = max / 100.f;
            }else{
                maxValueInterger = max;
            }
            CGFloat xMaxValue = [self maxValueFloat:maxValueInterger];
            if (max >= 1000) {
                return xMaxValue * 100;
            }
            return xMaxValue;
        }else{
            NSInteger maxValueInterger = 0;
...
-(NSInteger)maxValue:(NSInteger)maxValueInterger{
    NSInteger xMaxValue = maxValueInterger % (_row - 1);
    if (xMaxValue != 0) {
        xMaxValue = (_row - 1) - xMaxValue + maxValueInterger;
    }else{
...
-(CGFloat)maxValueFloat:(CGFloat)maxValueInterger{
    CGFloat xMaxValue = maxValueInterger / (_row - 1);
    return xMaxValue;
}
...
-(NSMutableArray *)addDataPointWith:(UIView *)view andArr:(NSArray *)DataArr andInterval:(CGFloat)interval{
    CGFloat height = self.chartScrollView.bounds.size.height - 13 - KCircleRadius1 / 2 - 3;
    //初始点
    ...
    for (int i = 0; i<arr.count; i++) {
        ...
        if (self.min && self.max) {
            ...
            if (x < 0) {
                x = 0;
            }else if (x > (self.max - self.min)){
                x = (self.max - self.min);
            }
        }
        float tempHeight = x / (interval * (_row - 1));
//        NSValue *point = [NSValue valueWithCGPoint:CGPointMake(xMargin * i + xMargin, (height - (height *(1 - tempHeight) + 13)) < KCircleRadius1 / 2 + 3 ? (height + 13) - KCircleRadius1 / 2 - 3 : height *(1 - tempHeight) + 13)];
        NSValue *point = [NSValue valueWithCGPoint:CGPointMake(xMargin * i + xMargin,(1 - tempHeight) * height + 13)];
        if (i == 0) {
//            NSValue *point1 = [NSValue valueWithCGPoint:CGPointMake(0 , (height - (height *(1 - tempHeight) + 13)) < KCircleRadius1 / 2 + 3 ? (height + 13) - KCircleRadius1 / 2 - 3 : height *(1 - tempHeight) + 13)];
            NSValue *point1 = [NSValue valueWithCGPoint:CGPointMake(xMargin * i + xMargin,(1 - tempHeight) * height + 13)];
...
-(void)addLeftViews{
    ...
    for (NSInteger i = 0;i< _row ;i++ ) {
        ...
        if ((self.min > 0 && self.isShowYtext) || (self.min == 0 && i > 0 && self.isShowYtext)){
            leftLabel.hidden = NO;
        }else{
...
-(NSString *)unitValue:(CGFloat)value{
    ...
    if (_unitStyle == LRSUnitGe || _unitStyle == LRSUnitDefault) {
        returnValue = value + self.min;
    }else if (_unitStyle == LRSUnitMoneyPercentageDefault){
        return [NSString stringWithFormat:@"%.0f%%",value + self.min];
    }else if (_unitStyle == 1) {
        returnValue = (value + self.min) / 1000;
    }else if (_unitStyle == 2){
        returnValue = (value + self.min) / 10000;
    }else if (_unitStyle == 3){
        returnValue = (value + self.min) / 100000000;
    }else if (_unitStyle == 4){
        if (value >= 100000000) {
            return [NSString stringWithFormat:@"%.1f亿%@",(value + self.min) / 100000000,self.unitName];
        }else{
            return [NSString stringWithFormat:@"%.1f万%@",(value + self.min) / 10000,self.unitName];
        }
    }
    if (returnValue >= 1000) {
    ...
    if (self.min && self.max) {
        return [NSString stringWithFormat:@"%.1f%@",returnValue,self.unitName];
    }else{
